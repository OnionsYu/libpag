...
struct PPTExportOptions {
  ...
  /**
   * Oversampling factor applied when a layer has to be baked to PNG. The Surface behind every bake
   * (masked layer, scrollRect fallback, blend/wide-gamut fallback, tiled pattern) is sized at
   * `ceil(logicalSize * rasterScale)` physical pixels, while the placed picture keeps the logical
   * EMU dimensions — the consumer stretches the denser bitmap over the same visible area, giving
   * a crisper result at the cost of file size.
   *
   * Valid range: (0, 4]. Values outside this range are clamped at the entry point — passing 0 or
   * a negative value would otherwise silently produce a zero-pixel surface. The default is 2.0
   * (i.e. @2x assets), matching the default of HTMLExportOptions::rasterScale.
   */
